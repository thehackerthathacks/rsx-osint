...
from modules.utils.http  import async_get, sync_get, base_headers
...
class BreachModule:
    ...
    async def _hibp(self, query: str, qtype: str):
        ...
        if qtype == "email":
            url = f"https://haveibeenpwned.com/api/v3/breachedaccount/{urllib.parse.quote(query)}?truncateResponse=false"
            hdrs = {"hibp-api-key": key, "User-Agent": "rsx-osint"} if key else {"User-Agent": "rsx-osint"}
            ...
            if resp.status_code == 200:
                try:
                    for b in resp.json():
                        ...
                        if self.store.add("haveibeenpwned", qtype, query,
                                f"{b.get('Name')} breach ({b.get('BreachDate','?')})",
                                {"leaked data": ", ".join(b.get("DataClasses",[]))[:80],
                                 "pwned count": f"{b.get('PwnCount',0):,}",
                                 "verified": str(b.get("IsVerified",False)),
                                 "description": desc}):
                            self.tui.result_card(self.store.count(),"haveibeenpwned",qtype,
                                f"{b.get('Name')} breach ({b.get('BreachDate','?')})",
                                {"leaked data": ", ".join(b.get("DataClasses",[]))[:80],
                                 "pwned count": f"{b.get('PwnCount',0):,}",
                                 "description": desc})
                ...
                self.tui.info("HIBP: email not found in any breach")
            elif resp.status_code == 429:
                self.tui.warn("HIBP: rate limited")

        elif qtype in ("password","hash"):
            raw = qtype == "hash" and len(query) == 40 and all(c in "0123456789abcdefABCDEF" for c in query)
            ...
            if resp and resp.status_code == 200:
                for line in resp.text.splitlines():
                    h, cnt = line.split(":")
                    if h == suffix:
                        sev = "CRITICAL" if int(cnt)>10000 else "HIGH" if int(cnt)>1000 else "MEDIUM"
                        if self.store.add("haveibeenpwned", qtype, query,
                                f"Found in {int(cnt):,} breach entries",
                                {"severity": sev, "method": "k-anonymity SHA1 range"}):
                            self.tui.result_card(self.store.count(),"haveibeenpwned",qtype,
                                f"Found in {int(cnt):,} breach entries",
                                {"severity": sev})
                        return
    ...
    async def _hibp_public(self, query: str, qtype: str):
        url = f"https://haveibeenpwned.com/unifiedsearch/{urllib.parse.quote(query)}"
        ...
        if resp and resp.status_code == 200:
            try:
                for b in resp.json().get("Breaches",[]):
                    ...
                    if self.store.add("haveibeenpwned", qtype, query,
                            f"{b.get('Name')} ({b.get('BreachDate','?')})", extra):
                        self.tui.result_card(self.store.count(),"haveibeenpwned",qtype,
                            f"{b.get('Name')} ({b.get('BreachDate','?')})", extra)
            except Exception: pass
    ...
    async def _breachdirectory(self, query: str, qtype: str):
        ...
        if resp.status_code == 200:
            try:
                data = resp.json()
                if data.get("found"):
                    for r in data.get("result",[])[:self.cfg.get("max_results_per_source",50)]:
                        pw = r.get("password") or "(hashed only)"
                        extra = {"breach source": r.get("sources",""), "sha1": r.get("sha1",""),
                                 "has plaintext": str(bool(r.get("password")))}
                        if self.store.add("breachdirectory", qtype, query, pw, extra):
                            self.tui.result_card(self.store.count(),"breachdirectory",qtype,pw,extra)
                else:
    ...
    async def _proxynova(self, query: str, qtype: str):
        ...
        url = f"https://api.proxynova.com/comb?query={urllib.parse.quote(query)}&start=0&limit=50"
        resp = await async_get(url, timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                data = resp.json()
                ...
                if lines:
                    self.tui.info(f"ProxyNova: {total:,} total entries")
                    for line in lines[:self.cfg.get("max_results_per_source",50)]:
                        u,_,p = line.partition(":")
                        extra = {"user": u.strip(), "password": p.strip(),
                                 "total in dump": str(total)}
                        if self.store.add("proxynova", qtype, query, line, extra):
                            self.tui.result_card(self.store.count(),"proxynova",qtype,line,extra)
                else:
                    self.tui.info("ProxyNova: no entries found")
            except Exception: pass
            ...
            try:
                ...
                if data.get("found"):
                    for src in data.get("sources",[]):
                        extra = {"date": src.get("date",""), "entries": str(src.get("entries",""))}
                        if self.store.add("leakcheck", qtype, query,
                                src.get("name","Unknown breach"), extra):
                            self.tui.result_card(self.store.count(),"leakcheck",qtype,
                                src.get("name","Unknown breach"), extra)
                else:
    ...
    async def _ghostproject(self, query: str, qtype: str):
        ...
        if resp and resp.status_code == 200:
            ...
            for row in soup.select("table tr")[1:21]:
                ...
                if len(cols) >= 2:
                    cred = f"{cols[0].get_text(strip=True)}:{cols[1].get_text(strip=True)}"
                    if self.store.add("ghostproject", qtype, query, cred, {}):
                        self.tui.result_card(self.store.count(),"ghostproject",qtype,cred,{})
    ...
    async def _snusbase(self, query: str, qtype: str):
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with s.post(url, json=payload, ssl=False,
                                  timeout=aiohttp.ClientTimeout(total=self.timeout)) as r:
                    if r.status == 200:
                        data = await r.json()
                        for db, entries in data.get("results",{}).items():
                            for entry in entries[:15]:
                                display = entry.get("email") or entry.get("username") or "entry"
                                extra = {"password": entry.get("password",""),
                                         "hash": entry.get("hash",""),
                                         "database": db,
                                         "ip": entry.get("ip","")}
                                if self.store.add("snusbase", qtype, query,
                                        f"{db} — {display}", extra):
                                    self.tui.result_card(self.store.count(),"snusbase",
                                        qtype, f"{db} — {display}", extra)
    ...
    async def _intelx(self, query: str, qtype: str):
        ...
        try:
            async with aiohttp.ClientSession(headers=hdrs) as s:
                payload = {"term": query, "maxresults": 20, "media": 0, "sort": 4, "terminate": []}
                async with s.post(f"{base}/intelligent/search", json=payload, ssl=False,
                    ...
                    if r.status == 200:
                        ...
                        if sid:
                            ...
                            async with s.get(
                                f"{base}/intelligent/search/result?id={sid}&limit=20&offset=0",
                                ssl=False
                            ) as r2:
                                if r2.status == 200:
                                    for rec in (await r2.json()).get("records",[]):
                                        extra = {"date": rec.get("date","")[:10],
                                                 "bucket": rec.get("bucket",""),
                                                 "size": str(rec.get("size",""))}
                                        if self.store.add("intelx", qtype, query,
                                                rec.get("name","record"), extra):
                                            self.tui.result_card(self.store.count(),"intelx",
                                                qtype, rec.get("name","record"), extra)
        except Exception as e:
    ...
    async def _leakix(self, query: str, qtype: str):
        ...
        if key:
            hdrs["api-key"] = key
        url = f"https://leakix.net/search?scope=leak&q={urllib.parse.quote(query)}&page=0"
        resp = await async_get(url, headers=hdrs, timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                for item in resp.json()[:15]:
                    extra = {"summary": item.get("summary","")[:100],
                             "host": item.get("host",""),
                             "date": item.get("time","")[:10],
                             "severity": item.get("severity","")}
                    if self.store.add("leakix", qtype, query,
                            item.get("event_source","leak entry"), extra):
                        self.tui.result_card(self.store.count(),"leakix",qtype,
                            item.get("event_source","leak entry"), extra)
            except Exception: pass
    ...
    async def _emailrep(self, query: str):
        ...
        if resp and resp.status_code == 200:
            try:
                d = resp.json()
                flags = d.get("details",{})
                extra = {"blacklisted": str(flags.get("blacklisted",False)),
                ...
                val = f"Reputation: {d.get('reputation','?').upper()}  |  Suspicious: {d.get('suspicious',False)}"
                if self.store.add("emailrep","email",query,val,extra):
                    self.tui.result_card(self.store.count(),"emailrep","email",val,extra)
            except Exception: pass
    ...
    async def _whatsmyname(self, query: str):
        ...
        async def check(site):
            uri = site.get("uri_check","").replace("{account}", urllib.parse.quote(query))
            if not uri:
                return
            async with sem:
            ...
            if r.status_code == ecode and (not estring or estring in r.text):
                extra = {"platform": site.get("name","?"), "category": site.get("cat",""),
                         "url": uri}
                if self.store.add("whatsmyname","username",query,uri,extra):
                    self.tui.result_card(self.store.count(),"whatsmyname","username",uri,extra)

        await asyncio.gather(*[check(s) for s in sites[:200]], return_exceptions=True)

    # ── Shodan InternetDB ─────────────────────────────────────────────────
    async def _shodan(self, query: str):
        self.tui.info("Shodan InternetDB — querying...")
        resp = await async_get(f"https://internetdb.shodan.io/{urllib.parse.quote(query)}",
                               timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                d = resp.json()
                ports = ", ".join(str(p) for p in d.get("ports",[]))
                extra = {"hostnames": ", ".join(d.get("hostnames",[])),
                         "cves": ", ".join(d.get("vulns",[])),
                         "tags": ", ".join(d.get("tags",[])),
                         "cpes": ", ".join(d.get("cpes",[])[:3])}
                val = f"Open ports: {ports or 'none'}"
                if self.store.add("shodan","ip",query,val,extra):
                    self.tui.result_card(self.store.count(),"shodan","ip",val,extra)
            except Exception: pass
    ...
    async def _virustotal(self, query: str, qtype: str):
        ...
        ep = {"ip":"ip_addresses","domain":"domains","hash":"files"}.get(qtype)
        if not ep:
            return
        url = f"https://www.virustotal.com/api/v3/{ep}/{urllib.parse.quote(query)}"
        resp = await async_get(url, headers={"x-apikey": key}, timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                attr  = resp.json().get("data",{}).get("attributes",{})
                stats = attr.get("last_analysis_stats",{})
                extra = {"reputation": str(attr.get("reputation","")),
                         "country": attr.get("country",""),
                         "as_owner": attr.get("as_owner",""),
                         "tags": ", ".join(attr.get("tags",[]))}
                val = f"Malicious: {stats.get('malicious',0)}/{sum(stats.values())} engines"
                if self.store.add("virustotal",qtype,query,val,extra):
                    self.tui.result_card(self.store.count(),"virustotal",qtype,val,extra)
            except Exception: pass

    # ── URLScan ───────────────────────────────────────────────────────────
    async def _urlscan(self, query: str, qtype: str):
        self.tui.info("URLScan.io — querying...")
        url = f"https://urlscan.io/api/v1/search/?q={urllib.parse.quote(query)}&size=10"
        resp = await async_get(url, timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                for r in resp.json().get("results",[])[:10]:
                    page = r.get("page",{})
                    extra = {"ip": page.get("ip",""), "country": page.get("country",""),
                             "server": page.get("server",""),
                             "scan date": r.get("task",{}).get("time","")[:10]}
                    val = page.get("url","")
                    if val and self.store.add("urlscan",qtype,query,val,extra):
                        self.tui.result_card(self.store.count(),"urlscan",qtype,val,extra)
            except Exception: pass
                ...
                for cert in resp.json()[:30]:
                    ...
                    for n in name.split("\n"):
                        ...
                        if n and n not in seen:
                            seen.add(n)
                            extra = {"issuer": cert.get("issuer_name","")[:60],
                                     "valid from": cert.get("not_before","")[:10],
                                     "valid until": cert.get("not_after","")[:10]}
                            if self.store.add("crt_sh",qtype,query,n,extra):
                                self.tui.result_card(self.store.count(),"crt_sh",qtype,n,extra)
            except Exception: pass

    # ── NumVerify / AbstractAPI ───────────────────────────────────────────
    async def _numverify(self, query: str):
        ...
        if key:
            url = f"http://apilayer.net/api/validate?access_key={key}&number={phone}&format=1"
        else:
            url = f"https://phonevalidation.abstractapi.com/v1/?api_key=free&phone={phone}"
        resp = await async_get(url, timeout=self.timeout)
        if resp and resp.status_code == 200:
            try:
                d = resp.json()
                extra = {"valid": str(d.get("valid","")),
                         "country": d.get("country_name","") or d.get("country",""),
                         "location": d.get("location",""),
                         "line type": d.get("line_type","") or d.get("type",""),
                         "carrier": d.get("carrier","")}
                val = f"{phone}  —  {d.get('carrier',d.get('carrier','?'))}"
                if self.store.add("numverify","phone",query,val,extra):
                    self.tui.result_card(self.store.count(),"numverify","phone",val,extra)
            except Exception: pass
